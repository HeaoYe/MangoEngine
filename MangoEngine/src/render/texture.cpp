...
namespace MangoEngine {
    std::shared_ptr<Texture> Texture::load_from_file(const char *filename) {
        auto texture = render_system->get_context().get_resource_factory_reference().create_texture(filename);
        return std::move(std::shared_ptr<Texture>(new Texture(texture.release())));
    }

    std::shared_ptr<Texture> Texture::generate() {
        auto texture = render_system->get_context().get_resource_factory_reference().create_empty_texture();
        return std::move(std::shared_ptr<Texture>(new Texture(texture.release())));
    }
    ...
    Texture::Texture(MangoRHI::Texture *raw_texture) : uuid(_current_uuid) {
        ...
        this->texture->destroy_before(&render_system->get_context());
        _current_uuid ++;
    }
}
